...
class ImageRegressionDataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None, normalize_mean=None, normalize_std=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # Ensure image is in RGB format (convert grayscale or RGBA to RGB)
        if image.mode != 'RGB':
        ...
        if self.normalize_mean is not None and self.normalize_std is not None:
            ...
            # normalize_mean and normalize_std should be tuples/lists of length 3
            mean = np.array(self.normalize_mean, dtype=np.float32).reshape(1, 1, 3)
            std = np.array(self.normalize_std, dtype=np.float32).reshape(1, 1, 3)
